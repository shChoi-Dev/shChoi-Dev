...
- **핵심 기여:**
    - **웹 성능 최적화:** `Thumbnailator` 라이브러리를 활용하여 상품 이미지 업로드 시 리사이징을 구현함으로써 **초기 로딩 속도 개선 및 서버 저장 효율성 증대**
...
### 🎓 Education
- **코멘토 | IT 대기업 현업 개발자와 함께하는 백엔드 개발 실무 과정(39차):**
    - **RESTful API 가이드라인 준수:** HTTP 메서드(GET, POST)와 URI 자원 설계를 통해 표준화된 API 인터페이스 구축
    - **데이터 통계 로직 구현:** MyBatis를 활용하여 대용량 데이터를 연도별, 월별 등으로 그룹화하는 복잡한 SQL 통계 쿼리 최적화
    - **글로벌 예외 처리 시스템:** `@RestControllerAdvice`를 도입하여 비즈니스 로직에서 발생하는 예외를 통합 관리하고 표준화된 에러 응답 객체 반환
